#+ name, include=FALSE
## name: eddm_sca_pairs_eftu.r
## date: 11/11/2016 

#'---
#'title: "Ensemble Difference Distance Matrices(eddm) analysis of EF-Tu Crystallographic Structures and Statistical Coupling Analysis(sca) of EF-Tu protein family (pfam PF00009)"
#'---
...
#+ load_pdbs
# load distance matrix, sca matrix and residue number/name/community of EF-Tu
load("/Users/hyangl/project/ras/results/2016/1027_dm/eftu/dmaa.RData")
...
#'
#' ## 1. EDDM Analysis of EF-Tu Crystallographic GTP-GDP State Structures 
#'
...
# general statistics of all-atom distance matrix of eftu structures in GTP/GDP states
dmaa_eftu_gtp = apply(dmaa_eftu385[,,grps==1],1:2,mean)
dmaa_eftu_gdp = apply(dmaa_eftu385[,,grps==2],1:2,mean)
# mean distance of all residue pairs
mean(dmaa_eftu_gtp[upper.tri(dmaa_eftu_gtp)]); mean(dmaa_eftu_gdp[upper.tri(dmaa_eftu_gdp)]);
# sd of all the distances
sd(dmaa_eftu_gtp[upper.tri(dmaa_eftu_gtp)]); sd(dmaa_eftu_gdp[upper.tri(dmaa_eftu_gdp)]);

#+ distribution of residue-residue distances, fig.cap='The distribution of the mean distances between all residue pairs'
layout(matrix(1:2, 1, 2))
hist(dmaa_eftu_gtp, xlab="distance (angstrom)", main="the GTP-bound state")
hist(dmaa_eftu_gdp, xlab="distance (angstrom)", main="the GDP-bound state")

#+ summary of eddm beta0.5
# The differences between GTP and GDP state
head(subseteddm_eftu_gxp_beta0.5)
# summary of intra and inter community pairs
tbl_eftu_gxp_beta0.5
# number of intra- and inter-community pairs
sum(diag(tbl_eftu_gxp_beta0.5)); sum(tbl_eftu_gxp_beta0.5[upper.tri(tbl_eftu_gxp_beta0.5)])

#+ SII - a3
# residue pairs identified between SII and a3
subseteddm_eftu_gxp_beta0.5[subseteddm_eftu_gxp_beta0.5[,"community_i"]=="SII" &
  subseteddm_eftu_gxp_beta0.5[,"community_j"]=="a3",-c(3:6)]

#+ a1/b1-b3 - a5
# residue pairs identified between a1/b1-b3 and a5
subseteddm_eftu_gxp_beta0.5[subseteddm_eftu_gxp_beta0.5[,"community_i"]=="a1/b1-b3" &
  subseteddm_eftu_gxp_beta0.5[,"community_j"]=="a5", -(3:6)]

#+ SII - a3 from sca top 100
# residue pairs between SII and a3 from top 100 sca
sca_eftu_100 <- sca_eftu[1:100,]
sca_eftu_100[sca_eftu_100[,"community_i"]=="SII" & sca_eftu_100[,"community_j"]=="a3",]
# residue pairs between a1/b1-b3 - a5
sca_eftu_100[sca_eftu_100[,"community_i"]=="a1/b1-b3" & sca_eftu_100[,"community_j"]=="a5",]

#'
#' ## 2. Comparison of EDDM and SCA results 
#'
#' EDDM returns residue pairs of the entire protein (this comparison only consider RasD).
#' SCA returns residue pairs of ONLY RasD (PF00009).
...
# number of intra- and inter-community pairs
sum(diag(tbl_eddm_eftu_gxp_100)); sum(tbl_eddm_eftu_gxp_100[upper.tri(tbl_eddm_eftu_gxp_100)])

#+ summary of sca 100
# summary of sca top 100
tbl_sca_eftu_100
# number of intra- and inter-community pairs
sum(diag(tbl_sca_eftu_100)); sum(tbl_sca_eftu_100[upper.tri(tbl_sca_eftu_100)])

#+ common pairs found by sca and eddm
# the index for D2 and D3 (only consider RasD)
...
library(rmarkdown)
render("eddm_sca_pairs_eftu.r", "pdf_document", clean=TRUE)
